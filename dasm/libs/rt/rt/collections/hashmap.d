...
struct SHM(Key, Value, Hasher = HashFunc!(Key), Comparer = HashComp!(Key))
{
    ...
    bool get(const ref Key key, Value* value)
    {
        uint hash = Hasher.hash(key);
        foreach(size_t i, Pair p; table)
        {
            if (p.hash == hash && Comparer.compare(p.key, key))
            {
                value = &p.value;
                return true;
            }
    ...
    void set(const ref Key key, const ref Value value)
    {
        uint hash = Hasher.hash(key);
