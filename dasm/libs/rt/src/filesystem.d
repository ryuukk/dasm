...
struct InputFile
{
    ...
    int size()
    {
        ...
        {
            assert(null != handle);
            long pos = ftell(cast(FILE*)handle);
    ...
    bool read(void* buffer, ulong size) {
        version(Windows)
        {
            assert(INVALID_HANDLE_VALUE != handle);
            DWORD readed = 0;
...
struct FS
{
    alias content_cb_t = void delegate(ulong, const(ubyte)*, bool);
    static AsyncHandle invalid_handle = { value: 0xffFFffFF };
    struct AsyncHandle
    {
        uint value = 0;
        bool is_valid() { return value != 0xffFFffFF; }
    }

    struct AsyncItem
    {
        enum Flags { FAILED = 1 << 0, CANCELED = 1 << 1 }
        content_cb_t cb;
        OutputStream data;
        char[256] path;
        ubyte flags = 0;
	...
	AsyncHandle get_content_async(string path, content_cb_t callback)
	{
        ...
        version(HAS_THREADS) mutex.enter();
        version(HAS_THREADS) scope(exit) mutex.exit();
        ...
        version(HAS_THREADS) semaphore.signal();
    ...
    void process_callbacks()
    {
		...
		for(;;) {
            ...
            version (HAS_THREADS) mutex.enter();
			if (finished.empty()) {
				version (HAS_THREADS) mutex.exit();
				break;
			}
			...
			if(!(item.is_canceled())) {
				item.cb(item.data.size, item.data.data, !item.is_failed());
			}
