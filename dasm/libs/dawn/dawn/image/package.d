...
{
    return cstdio.fseek(cast(cstdio.FILE*) st, off, cstdio.SEEK_CUR);
}

int filewrite(void* st, ubyte[] buf)
{
    return cast(int) cstdio.fwrite(buf.ptr, 1, buf.length, cast(cstdio.FILE*) st);
}
...
IFInfo read_info(in char[] fname)
{
    ...
    cstdio.FILE* f = cstdio.fopen(tmp.ptr, "rb");
    tmp.drop();
    if (!f) {
        info.e = ERROR.fopen;
        return info;
...
IFInfo read_info(cstdio.FILE* f)
{
...
/// Reads from f which must already be open. Does not close it afterwards.
IFImage read_image(cstdio.FILE* f, in int c = 0, in int bpc = 8)
{
    IFImage image;
    Read io = { cast(void*) f, &fileread, &fileseek };
...
ubyte write_image(char[] fname, int w, int h, in ubyte[] buf, int reqchans = 0)
{
    ...
    if (!tmp.ptr)
        return ERROR.oom;
    cstdio.FILE* f = cstdio.fopen(tmp.ptr, "wb");
    tmp.drop();
    if (!f)
        return ERROR.fopen;
...
ubyte write_image(int fmt, cstdio.FILE* f, int w, int h, in ubyte[] buf, int reqchans = 0)
{
